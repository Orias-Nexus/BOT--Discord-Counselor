...
The system utilizes a relational schema to manage state:

- **servers**: Core configuration (timeout settings, role assignments, subscription status).
- **members**: Link between users and servers, tracking local EXP, levels, and status (Warn/Mute/Lock).
- **users**: Global user profiles tracking cross-server EXP and levels.
- **channels**: Managed categories for Voice Creator and real-time Server Statistics.
- **embeds**: Custom embed templates defined by server administrators.
- **messages**: Mapping of automated message types (Greeting, Leaving, etc.) to specific channels and embeds.
- **levels**: Global experience threshold definitions.

## Core Features (Directive Scripts)

The logic is modularized into scripts handling:

- **Server Management**: System info, status role/timeout configurations, and stats tracking.
- **Member Moderation**: Rename, move, warn, mute, lock, and kick operations with automated expiration.
- **Channel Automation**: Automatic creation of private voice rooms and category cloning.
- **Engagement**: Experience and Leveling systems (both global and server-local).
- **Custom Content**: Dynamic Embed Builder and automated message triggers.
