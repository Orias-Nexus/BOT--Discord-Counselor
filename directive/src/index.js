...
import {
  ...
  isButtonModalScript,
  isButtonDeferUpdate,
} from "./actions/handleAction.js";
import {
  parseModalCustomId,
  getModalInputIds,
  SCRIPTS_NEED_MODAL,
} from "./actions/modalConfig.js";
...
import {
  isServerStatsSelectId,
  doSetServerStats,
} from "./scripts/setServerStats.js";
...
client.once(Events.ClientReady, (c) => {
  ...
  console.log(
    `[startup] REDIS_STORAGE_URL: ${env.redisStorageUrl || "(not set)"}`,
  );
...
client.on(Events.InteractionCreate, async (interaction) => {
  if (interaction.isAutocomplete()) {
    ...
    if (useEmbedId || useEmbedName) {
      ...
      try {
        const list = await import("./api.js").then((m) =>
          m.listEmbeds(guildId),
        );
  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId?.startsWith(EMBED_APPLY_SELECT_PREFIX)) {
      const embedId = interaction.customId.slice(
        EMBED_APPLY_SELECT_PREFIX.length,
      );
      ...
      try {
        ...
        if (guildId) {
          ...
          for (const type of types) {
            await api.setMessageEmbed(
              guildId,
              type,
              selected.includes(type) ? embedId : null,
            );
          }
        }
        await interaction
    ...
    if (isServerStatsSelectId(interaction.customId)) {
      try {
        ...
        const channelsIdx =
          values.length > 0 ? parseInt(values.join(""), 10) : 0;
        ...
        if (!interaction.deferred)
          ...
          await interaction
            .editReply({ content: "Error.", components: [] })
            .catch(() => {});
      }
      return;
    }
  ...
  if (interaction.isButton()) {
    ...
    if (!needsModal) {
      try {
        ...
        if (isUnknownInteraction(err)) {
          console.warn(
            "[InteractionCreate] Button defer 10062 - token expired, click the button again.",
          );
          return;
        }
        throw err;
    ...
    const interactionAge = interaction.createdTimestamp
      ? receivedAt - interaction.createdTimestamp
      : 0;
    try {
      const handled = await handleAction(interaction, client, {
        receivedAt,
        interactionAge,
      });
      if (handled) return;
      ...
      try {
        ...
        if (interaction.message) {
          if (payload) {
            ...
            const componentsOnly = resetComponentsOnly(
              scriptName,
              interaction,
              actionContext,
            );
            if (componentsOnly) {
              await interaction.message
                .edit({ components: componentsOnly })
                .catch(() => {});
            }
          }
        }
  ...
  if (interaction.isChatInputCommand()) {
    ...
    const slashUsesEmbed =
      scriptName && EMBED_BY_SCRIPT[scriptName]?.replyType === "embed";
    try {
      await interaction.deferReply({
        flags: slashUsesEmbed ? 0 : MessageFlags.Ephemeral,
      });
    } catch (err) {
      if (isUnknownInteraction(err)) {
        console.warn(
          "[InteractionCreate] Token expired (10062) - try the command again in a few seconds.",
        );
        return;
      }
      throw err;
    ...
    if (!scriptName) {
      await interaction
        .editReply({ content: "Unknown command." })
        .catch(() => {});
      return;
    }
