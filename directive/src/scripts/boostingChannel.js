...
export async function run(interaction, client, actionContext) {
  await setEventChannel(interaction, client, actionContext, 'Boosting');
=======
...
export async function run(interaction, client, actionContext) {
  ...
  try {
    ...
    const text = await resolveString('Completed Set {channel_name} as Boosting Channel', { guild, channel });
    await interaction.editReply({ content: text, flags: MessageFlags.Ephemeral }).catch(() => {});
  } catch (err) {
    console.error('[BoostingChannel]', err);
