...
/**
 * Core warn logic — shared by slash commands and web dashboard.
...
export async function execute({ guild, member, server, executorUser = null }) {
  ...
  if (server?.role_warn) {
    ...
    if (role) {
      ...
      if (err) return { ok: false, error: 'Failed to add warn role — check bot permissions.' };
    }
  }
  ...
  await api.setMemberStatus(guild.id, member.id, 'Warn', expiresAt);
