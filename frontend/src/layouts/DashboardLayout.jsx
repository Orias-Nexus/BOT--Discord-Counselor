    ...
    <div className="bg-surface text-on-surface font-body selection:bg-primary/30 min-h-screen">
      ...
      <aside className="h-screen w-64 fixed left-0 top-0 flex flex-col bg-surface-container-lowest border-r border-white/5 z-60 font-['Manrope'] font-medium">
          ...
          {SIDEBAR_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                className={
                  isActive
                    ? "flex items-center gap-3 px-4 py-3 bg-linear-to-r from-[#dab9ff]/20 to-transparent border-l-4 border-[#dab9ff] text-[#dab9ff] hover:translate-x-1 duration-200 group"
        ...
        <div className="p-4 border-t border-white/5 space-y-1">
          <button type="button" className="flex items-center gap-3 px-4 py-2 text-slate-500 hover:text-slate-200 transition-all text-sm cursor-pointer border-none bg-transparent w-full text-left">
      ...
      <header className="fixed top-0 left-64 right-0 z-50 flex items-center justify-between px-8 py-4 bg-[#10131a]/40 backdrop-blur-xl shadow-[0_24px_48px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4 text-slate-400">
            <button type="button" aria-label="Notifications" className="hover:bg-white/5 p-2 rounded-full transition-all active:scale-90">
              <span className="material-symbols-outlined">notifications</span>
            </button>
            <button type="button" aria-label="Settings" className="hover:bg-white/5 p-2 rounded-full transition-all active:scale-90">
