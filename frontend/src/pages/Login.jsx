...
export default function Login() {
  ...
  useEffect(() => {
    // Check if there is a token in the URL (from backend redirect)
    const urlParams = new URLSearchParams(window.location.search);
    const token = urlParams.get('token');
    
    // In a real scenario, you'd decode the JWT or fetch /api/auth/me to get the user block
    if (token) {
      login(token, { name: 'Admin User' }); // Placeholder user data for now
      // Clean up URL
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-zinc-950 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-zinc-900 via-zinc-950 to-black">
