...
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION "DiscordCounselor".create_embeds_and_messages()
RETURNS TRIGGER
LANGUAGE plpgsql
...
DECLARE
  ...
  id_leveling  uuid;
  id_logging   uuid;
  j_greeting  jsonb := '{"title":"**Greeting! {user_name}!**","description":"**Welcome to {server_name}!**","color":"#57F287","timestamp":null,"author":{"name":"✦ {server_name}","icon_url":"{server_icon}"},"thumbnail":{"url":"{user_avatar}"},"image":{"url":"{server_banner}"},"fields":[{"name":"Your Position","value":"{server_membercount}","inline":true},{"name":"Rule Channel","value":"{rule_channel}","inline":true},{"name":"Small Rule","value":"Just enjoy your time here!","inline":false}],"footer":{"text":"Thank you for joining us!","icon_url":"{server_icon}"}}'::jsonb;
  j_leaving   jsonb := '{"title":"**Goodbye! {user_name}!**","description":"**Goodbye from {server_name}!**","color":"#57F287","timestamp":null,"author":{"name":"✦ {server_name}","icon_url":"{server_icon}"},"thumbnail":{"url":"{user_avatar}"},"image":{"url":"{server_banner}"},"fields":[],"footer":{"text":"See you next time!","icon_url":"{server_icon}"}}'::jsonb;
  j_boosting  jsonb := '{"title":"**Thank You! {user_name}!**","description":"**You are now boosting {server_name}!**","color":"#57F287","timestamp":null,"author":{"name":"✦ {server_name}","icon_url":"{server_icon}"},"thumbnail":{"url":"{user_avatar}"},"image":{"url":"{server_banner}"},"fields":[],"footer":{"text":"Thank you for boosting us!","icon_url":"{server_icon}"}}'::jsonb;
  j_leveling  jsonb := '{"title":"**Level Up! {user_name}!**","description":"**Congratulations on leveling up in {server_name}!**","color":"#57F287","timestamp":null,"author":{"name":"✦ {server_name}","icon_url":"{server_icon}"},"thumbnail":{"url":"{user_avatar}"},"image":{"url":"{server_banner}"},"fields":[{"name":"Your Level","value":"{member_level}","inline":true},{"name":"Your EXP","value":"{member_exp}","inline":true},{"name":"Your Rank","value":"{member_rank}","inline":true}],"footer":{"text":"Great progress!","icon_url":"{server_icon}"}}'::jsonb;
  j_logging   jsonb := '{"title":"**Log — {server_name}**","description":"**An event has been logged.**","color":"#57F287","timestamp":null,"author":{"name":"✦ {server_name}","icon_url":"{server_icon}"},"thumbnail":{"url":"{user_avatar}"},"image":{"url":null},"fields":[{"name":"Member","value":"{user_name}","inline":true},{"name":"Action","value":"Logged","inline":true},{"name":"Details","value":"See audit log for more info.","inline":false}],"footer":{"text":"Server Log","icon_url":"{server_icon}"}}'::jsonb;
BEGIN
...
COMMENT ON FUNCTION "DiscordCounselor".create_embeds_and_messages() IS 'Trigger: sau INSERT servers, tạo 5 embeds (template mặc định) + 5 messages (embed_id trỏ tới 5 embeds).';