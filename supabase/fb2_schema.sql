-- =============================================================================
-- Schema Supabase — Theo ARCHITECTURES.json (schema "DiscordCounselor")
...
COMMENT ON
TABLE "DiscordCounselor".levels IS 'Định nghĩa các mốc điểm yêu cầu cho các level';
...
COMMENT ON
TABLE "DiscordCounselor".servers IS 'Thông tin server (Discord guild)';
...
COMMENT ON
TABLE "DiscordCounselor".embeds IS 'Chỉ embed do người dùng tạo; embed của lệnh hardcode trong code';
...
COMMENT ON
TABLE "DiscordCounselor".messages IS 'Cấu hình tin nhắn theo loại; channel_id NULL = chưa cấu hình';
...
COMMENT ON
TABLE "DiscordCounselor".users IS 'Thông tin user; user_exp/user_level đối chiếu bảng levels';
...
COMMENT ON
TABLE "DiscordCounselor".members IS 'Thông tin member trong server; user_id tương ứng user_idu (FK users.user_id)';
...
COMMENT ON
TABLE "DiscordCounselor".channels IS 'Danh mục quản trị: Creator (1 kênh trigger) hoặc Stats (n kênh theo channels_idx).';
